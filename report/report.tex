...
\begin{enumerate}[label=(\arabic*),leftmargin=2em]
 ...
 \item 记忆模型的 $\phi$ 扫描 31/31 在当前积分范围内未灭绝，其中 11 个情景未通过长期收敛诊断，不能把有限积分结果误写为稳定平衡。
 \item 六模型扩展中不同序列由不同恐惧通道胜出，不能据此认定某一种恐惧机制普遍正确。
 \item 预测误差降低只能支持模型的预测能力，不能单独证明恐惧机制存在，也不能用于估计或比较真实恐惧强度。$\phi$ 在 15 条序列中多次触边；$M(0)$ 与 $\delta$ 的 profile 分析（覆盖 3 条代表序列）显示 $M(0)$ 不可辨识、$\delta$ 训练最优 $\neq$ 验证最优。三者共同表明恐惧与记忆参数缺乏可靠的可辨识性，但 profile 结论仅限于已分析的 3 条代表序列。
