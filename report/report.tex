...
结果表明，等效抑制下不同恐惧机制产生不同的长期种群响应，结论具有明显的机制路径依赖性。记忆模型的 $\phi$ 扫描 31/31 在当前积分范围内未灭绝，其中 11 个情景尚未通过长期收敛诊断。15 条真实种群序列中，含恐惧模型在留后预测上各有胜负，但 baseline 在 13 条序列上获 AICc 支持。代表序列的 profile 分析同时显示恐惧参数多数无法可靠辨识。因此，复杂模型可以改善部分序列的预测，但预测改善不能直接解释为恐惧机制真实存在，也不能支持对真实恐惧强度的稳定估计。
