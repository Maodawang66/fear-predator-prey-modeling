...
\[
 ...
 \frac{2p(p+1)}{n-p-1}.
\]
其中 $p$ 为实际拟合参数数目，$n=2n_{\mathrm{train}}$ 为猎物与捕食者训练残差的总数。该计算将归一化残差近似视为同方差 Gaussian 残差；生态时间序列中的自相关、过程噪声和观测误差未被显式建模，因此 AICc 仅作为当前候选模型集合内的相对比较指标。单次末尾 20\% 留后同样可能对切分点敏感。
...
15 条主序列包含哺乳类、鱼类与浮游动物，详见附录 A。正式基础比较逐序列拟合 baseline 与 fear-memory；核心递进比较加入 fear-instant，完整扩展比较再加入 fear-saturating、fear-foraging 与 fear-handling。训练段为前 80\%，验证段为后 20\%；验证预测不是从验证起点重新初始化，而是从整条序列初始状态连续积分。比较顺序为：
...
baseline 逐序列拟合 6 个动力学参数（$r,K,a,\theta,e,\mu$）。五个恐惧候选模型均在相同 Holling II 参数基础上增加 1 个主要恐惧参数，因此各拟合 7 个参数；fear-memory 固定 $\delta=1$，其新增拟合参数为 $\phi$。fear-saturating 的 $h$ 固定为对应序列训练段捕食者中位数；这不同于理论标准化实验中取参考平衡点捕食者密度的设定。初始状态取猎物与捕食者首次同时为正的观测，不作为自由参数；此前导零观测若直接作为 ODE 初值，会使模型陷入 $(0,0)$ 吸收态。该规则仅使 GLERL 删除前 3 条观测并从 $(3.753,0.064)$ 开始，其余正式序列仍从首条观测开始。对 fear-memory 进一步设 $M(0)=y(0)$，使初始记忆与首个可观测捕食压力一致，避免引入弱可辨识的初始记忆状态。固定 $\delta=1$ 提供统一的记忆时间尺度（对应记忆半衰期 $\ln 2\approx 0.69$ 个序列时间单位），防止在短序列中与 $\phi$ 相互补偿导致两个恐惧参数均不可靠。
...
为检验上述记忆设定是否主导结果，本文还对 $M(0)$ 与 $\delta$ 进行补充 profile。在离散网格上固定目标参数并局部重新优化其余参数，再使用渐近 $\chi^2_1$ 阈值得到近似 95\% profile 集。$M(0)$ 的变化通常可被其余参数补偿，14/15 条序列的近似 95\% profile 集触及 $M(0)/y(0)=5$ 的扫描上界。若在留后段上探索性选择最优 $\delta$，相对固定 $\delta=1$ 的验证 RMSE 改善中位数约为 2.1\%；Windermere 北湖盆与 Isle Royale 的改善较大，因此不能声称 $\delta$ 对所有序列均不重要。完整结果见附录 E。
...
核心比较限定于 baseline、fear-memory 与 fear-instant 三模型，依次比较无恐惧、带记忆繁殖抑制与瞬时繁殖抑制。三者在真实数据拟合中分别优化参数；理论实验中的共同 $\phi$ 校准不用于真实拟合。表\,\ref{tab:wins} 汇总三模型的序列级预测结果。fear-memory 与 fear-instant 各在 6 条序列的留后验证上单独最优，baseline 在 3 条上最优；但 baseline 在 10 条序列上取得最低 AICc。修正零初值后，GLERL 由退化并列变为 fear-instant 留后最优、baseline AICc 最优。fear-memory 与 fear-instant 的逐序列 AICc 差绝对值中位数约为 0.979，未显示任一繁殖抑制结构获得一致支持。因此，不可仅凭个别序列的 AICc 高低宣称"记忆滞后不重要"或"瞬时恐惧更真实"。按研究聚合后，模型胜负分布依然不支持任何通道的普遍优越性（完整研究级胜负见附录 D）。
...
\begin{table}[H]
 ...
 \begin{tabular}{lcccc}
  ...
  \midrule
  留后验证归一化 RMSE 最优 & 3 & 6 & 6 & 0\\
  AICc 最优 & 10 & 1 & 4 & 0\\
  \bottomrule
 \end{tabular}
\end{table}

(c) 按相同训练段归一化、20\% 连续多步留后和固定种子多起点优化口径扩展到全部六模型（新增 fear-saturating、fear-foraging、fear-handling）后，90 次拟合均达到 success 或 usable\_limit 状态并进入主比较，其中 43 次为 success，47 次为 usable\_limit。六模型在序列级留后最优分布的离散程度高于三模型，但结论相同：无单一恐惧通道在留后预测和 AICc 上一致胜出。完整六模型胜负表、热力图及研究级聚合分析见附录 D。

为检验胜负是否为微小数值波动所致，进一步计算每条序列的 baseline 与最佳恐惧模型之间的 $\Delta\mathrm{AICc}$。结果表明，6/15 条序列的 $|\Delta\mathrm{AICc}|<2$，胜负在实质上可忽略；仅 3 条序列的 $|\Delta\mathrm{AICc}|>7$（强证据），其中 1 条支持 baseline、2 条支持恐惧模型。其余序列虽可能存在 AICc 最优模型，但证据差异未达到强证据阈值。

研究级敏感性分析（取研究内序列的指标中位数，将 Andrén 七区、Killifish 三站、Windermere 两湖盆各聚合为 1 个研究级单位，共 6 个研究级比较单位）显示，留后验证最优模型分布为 baseline 2、fear-instant 2、fear-memory 1、fear-handling 1；AICc 最优分布为 baseline 4、fear-memory 1、fear-instant 1。取组内最佳与最差聚合方式时，胜负分布更加分散，但”无单一通道一致胜出”的模式不变。因此，序列非独立性不改变本文的主要定性结论。完整六模型胜负表及研究级聚合结果见附录 D。

作为优化状态敏感性诊断，若仅保留 success 拟合，Windermere South 没有任何模型可比较，其余 14 条序列中每条保留的候选模型集合也不同。因此 success-only 结果不能形成候选集合一致的六模型胜负统计；其留后赢家仍分散于五种模型，AICc 赢家分散于六种模型，未出现单一通道一致胜出。该诊断同时说明，主胜负计数仍依赖 usable\_limit 解，解释时需保留这一计算不确定性。
...
\begin{table}[H]
 ...
 \begin{tabular}{p{0.16\textwidth}p{0.35\textwidth}p{0.35\textwidth}}
  ...
  \midrule
  恐惧强度估计 & fear-memory 的 $\phi$ 在 2/15 条序列中触及搜索边界；全序列 $M(0)$ 与 $\delta$ profile 显示记忆相关参数存在宽可接受范围 & 将 fitted $\phi$ 直接解释为真实恐惧强度；在缺少独立机制观测时跨序列比较恐惧抑制程度\\
  \midrule
...
为检验 fear-memory 的恐惧与记忆参数是否被数据可靠识别，本文对 $\phi$、$M(0)$ 和 $\delta$ 进行补充诊断。$\phi$ 的边界敏感性以及 $M(0)$、$\delta$ 的 profile 均覆盖全部 15 条序列；正文展示 3 条具有不同数据特征的代表序列，其余结果汇总于附录 E。
...
进一步分析各序列拟合所得 $\phi$ 与 baseline--fear-memory 的 $\Delta\mathrm{AICc}$ 关系。AICc 支持 baseline 的 13 条序列中，fitted $\phi$ 的中位数约为 0.00926；fear-memory 获得 AICc 支持的 2 条序列中，中位数约为 0.00196。当前样本未显示模型支持程度与 fitted $\phi$ 之间存在简单的单调关系，因此不能把 AICc 对 baseline 的支持直接等同于 $\phi$ 接近零，也不能把较大的 fitted $\phi$ 解释为更强的机制证据。$\Delta\mathrm{AICc}$ vs fitted $\phi$ 散点图见附录 E。

$\phi$ 在 2/15 条序列的拟合中触及搜索边界。更一般地，单个 fitted $\phi$ 仍可能与其余动力学参数相互补偿，不能仅凭点估计将其解释为"识别到的恐惧强度"。
...
$M(0)/y(0)$ profile 在 $[0,5]$ 上普遍平坦，14/15 条序列的近似 95\% profile 集触及扫描上界，表明初始记忆的变化通常可由其他动力学参数补偿。触及上界表示可接受范围可能延伸到扫描区间之外，不应解释为精确置信区间端点。
...
本文的理论实验与数据拟合之间存在一种有意义的张力，这与文献中反复出现的方法论问题相呼应。Wang 等（2019）和 Zanette 等（2003）的工作表明恐惧效应在原理上可以改变种群动力学；Preisser 等（2007）和 Peacor \& Werner（2001）的系统综述确认了 NCE 的生态重要性。这些理论模型关心的是\textbf{可能存在什么}——它们证明，一旦恐惧以某种机制进入动力学方程，确实可以产生可观察的种群响应。然而，当参数被校准到真实数据时，问题转变为\textbf{在当前数据中实际被支持的是什么}。本文的六模型比较和全序列参数 profile 正是从这一角度切入：含恐惧模型可在部分序列上改善留后预测，但 AICc 在多数序列上支持较简单的 baseline，且 $M(0)$ 与 $\delta$ 的宽 profile 表明恐惧相关参数可被其他动力学参数补偿。因而，当前诊断不足以支持对真实恐惧强度的可靠估计。这一发现与 Peacor 等（2022）的综述结论在精神上一致：TMIE/NCE 的证据来自实验操作，不能直接等同于从宏观种群时间序列拟合得到的恐惧强度。

数值实验支持”机制路径决定定性方向”，数据拟合支持”含恐惧模型能够改善部分序列的留后预测”，但二者都不足以证明真实系统存在相应恐惧机制。这一结论有三项独立的方法论依据。第一，15 条序列中 Andrén 七区、Killifish 三站和 Windermere 两湖盆存在研究内重复，统计独立性有限，不能视为 15 个独立生态证据。第二，AICc 在 baseline--fear-memory 两模型比较中有 13/15 条支持 baseline，在核心三模型和完整六模型比较中 baseline 分别有 10/15 与 8/15 条取得最低 AICc；留后预测优势与复杂度惩罚后的模型选择之间存在分歧。第三，记忆相关参数存在宽 profile，且 Windermere 案例揭示了外部人为干预（刺网移除）对封闭 ODE 假设的违反——当捕食者被人为压制而猎物先爆发后自然回落时，模型会通过参数畸变或额外结构灵活性补偿未建模过程，留后预测完全失效。综合这三点，预测能力与机制识别必须分开解释。
...
\textbf{预测层面：含恐惧模型在部分序列上改善留后预测，但无单一通道一致胜出。}在 15 个拟合单位（序列级）的六模型统一比较中，留后验证最优的模型因序列而异；AICc 标准下 baseline 在 10/15 条序列上最优（三模型口径），扩展至六模型后为 8/15。该结果不支持"添加恐惧一定能改善种群预测"的普遍预期，也不支持"某一种恐惧机制普遍正确"。六个研究级单位的敏感性分析不改变这一结论。证据强度为中等——15 个拟合单位覆盖哺乳类、鱼类和浮游动物，但部分序列来自同一研究设计，统计独立性有限；同时，六模型拟合中 47/90 为 usable\_limit，胜负统计仍包含优化不确定性。

\textbf{机制层面：预测改善不能等同于恐惧机制的识别。}含恐惧模型在某些序列上留后预测更优，但三点限制因素决定了不能将此解释为"数据中存在恐惧效应"的正面证据。第一，AICc 在多数序列上支持不含恐惧的 baseline，与留后预测优势形成分歧。第二，fear-memory 的 $\phi$ 在 2/15 条序列中触及边界，全序列 $M(0)$ 与 $\delta$ profile 显示记忆相关参数存在宽可接受范围——预测改善可能来自结构增加的灵活性，而非恐惧机制的生态真实性。第三，Windermere 序列的案例表明，当数据包含外部人为干预（刺网移除）时，封闭 ODE 模型通过参数畸变补偿未建模过程，拟合误差不能完全归因于模型结构的生态合理性。三种诊断从不同角度共同限定了本文能够提出的机制结论。
...
\begin{table}[H]
 ...
 \begin{tabular}{lll}
  ...
  \midrule
  汇总 & base 8；memory 7；并列 0 & base 13；memory 2\\
  \bottomrule
 \end{tabular}
\end{table}
...
六模型序列级留后赢家汇总为 baseline 3、fear-memory 5、fear-instant 3、fear-foraging 1、fear-handling 3；AICc 赢家汇总为 baseline 8、fear-memory 1、fear-instant 2、fear-saturating 1、fear-foraging 3。
...
\begin{table}[H]
 ...
 \begin{tabular}{lll}
  ...
  \midrule
  Andrén & fear-memory & fear-instant\\
  ...
  Komi lynx--hare & baseline & fear-memory\\
  \midrule
  汇总 & base 2；memory 1；instant 2；handling 1
         & base 4；memory 1；instant 1\\
  \bottomrule
 \end{tabular}
\end{table}

六模型 90 次拟合中，43 次为 success，47 次为 usable\_limit。仅保留 success 时，Windermere South 无模型可比较，其他序列保留的候选集合也不一致；因此 success-only 结果仅用于提示优化不确定性，不作为替代胜负表。
