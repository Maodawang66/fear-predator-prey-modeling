...
\begin{table}[H]
  ...
  \begin{tabular}{p{1.0cm}p{2.4cm}p{2.2cm}p{3.6cm}p{3.8cm}}
    \toprule
    ...
    $\eta$ 中位数$\neq$“物种恐惧排序”；裸 $k$ 受 $p,q$ trade-off 影响 \\
    B &
    Peacor/08/10/07 实验与元分析 &
    TMIE/NCE 占比、活动度/觅食抑制 &
...
\begin{table}[H]
  ...
  \begin{tabular}{clllp{4.2cm}}
    \toprule
    编号 & 数据集 & 类别 & 平台/DOI & 本项目角色 \\
    \midrule
    01 & Andrén 猞猁—狍 & A & Dryad 10.5061/dryad.9zw3r22mq & 7 区主拟合序列 \\
    02 & Killifish—食蚊鱼 & A & Dryad 10.5061/dryad.qjq2bvqpw & 三站点主拟合 \\
    03 & Hudson Bay 雪兔—猞猁 & A & 经典教学数据 & 周期动力学标定 \\
    04 & 密歇根湖浮游 & A & Dryad 10.5061/dryad.bh688ft & 入侵—浮游标定 \\
    07 & LTER 威斯康星湖鱼 & A & knb-lter-ntl.356 & 追加三对鱼类拟合 \\
    08 & 珊瑚礁恐惧×摄食 & B & Dryad 10.5061/dryad.vdncjsxtf & 觅食抑制 $\psi$ 先验 \\
    09 & 狐狸—野犬恐惧景观 & B & Dryad 10.5061/dryad.1rn8pk0x6 & 行为恐惧代理 \\
    10 & 豆娘捕食者线索 & B & Dryad 10.5061/dryad.bg79cnpj2 & 活动度抑制先验 \\
    12 & Peacor 捕食风险元分析 & B & Dryad 10.5061/dryad.ffbg79cxt & PLP\_studies.csv，TMIE/NCE 先验 \\
    05 & GPDD 全球种群库 & C & KNB 10.5063/F1BZ63Z8 & 大样本拓展储备 \\
    \bottomrule
  \end{tabular}
\end{table}
...
GPDD（KNB 10.5063/F1BZ63Z8）含 \textbf{5156} 条序列元数据、\textbf{约 18 万}种群观测记录，覆盖 1896 分类单元与全球多生境。本项目已完成数据包本地化与 \texttt{dataset\_catalog.json} 索引；\textbf{当前正文结果}基于 A 类 12 条精细标定 + B 类机制对照。\textbf{后续计划：}由 \texttt{gpdd\_series\_metadata} 与 \texttt{gpdd\_taxa} 筛选候选捕食—猎物对，批量运行\modeltrio{}，绘制类群—$\eta$ 分布，检验 B--D+恐惧 在哺乳/鸟类/鱼类等类群上的泛化边界（与第\,\ref{sec:results-calib} 节图\,\ref{fig:group} 衔接）。
...
\begin{enumerate}[label=\textbf{P\arabic*：},leftmargin=2em]
  ...
  \item \textbf{可复现性：}原始数据存 \texttt{data/raw/}；Peacor 使用 \texttt{12\_peacor\_risk\_meta/PLP\_studies.csv}（64 篇 PLP，自 Dryad 导出）；拟合输出 CSV 与深挖图存 \texttt{results/} 子目录。
\end{enumerate}
