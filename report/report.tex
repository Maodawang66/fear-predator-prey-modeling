...
结果表明，标准化基准下不同恐惧机制产生不同的长期种群响应，表现出明显的机制路径依赖性。记忆模型的 $\phi$ 扫描 31/31 在当前积分范围内未灭绝，其中 11 个情景尚未通过长期收敛诊断。15 个拟合单位（序列级）中，留后预测的最优模型随序列变化；在 baseline 与 fear-memory 的两模型比较中，baseline 在 13 条序列上获 AICc 支持，扩展到六模型后则在 8 条序列上取得最低 AICc。全序列 profile 分析同时显示记忆相关参数普遍存在宽可接受范围。因此，复杂模型可以改善部分序列的预测，但预测改善不能直接解释为恐惧机制真实存在，也不能支持对真实恐惧强度的稳定估计。
...
这些工作共同提示两项关键的方法要求。其一，不同恐惧机制不能在各自默认参数下直接比较——参数选择的任意性会掩盖机制结构的真实差异；应相对无恐惧基线设置标准化抑制基准。该基准可减少通道内部强度差异，却不能使繁殖抑制与捕食率抑制等不同过程量完全可通约。其二，训练误差的降低不是机制识别的充分证据——复杂模型通常更容易通过额外自由度降低训练误差；真实数据比较还需留后预测、复杂度惩罚（AICc）、优化状态过滤和参数可辨识性诊断。本文据此区分“模型能否预测轨迹”与“数据能否识别恐惧机制”。
...
15 条主序列包含哺乳类、鱼类与浮游动物，详见附录 A。正式基础比较逐序列拟合 baseline 与 fear-memory；完整扩展比较再加入 fear-instant、fear-saturating、fear-foraging 与 fear-handling。训练段为前 80\%，验证段为后 20\%；验证预测不是从验证起点重新初始化，而是从整条序列初始状态连续积分。模型比较与诊断依次报告：
\begin{enumerate}[label=(\arabic*),leftmargin=2em]
 \item 过滤失败拟合；
 \item 以连续多步留后验证 RMSE 比较预测能力；
 \item 以统一归一化训练残差下的 AICc 比较复杂度校正后的模型支持；
 \item 以训练归一化 RMSE 描述拟合质量；
 \item 检查参数触边。
\end{enumerate}
...
为检验上述记忆设定是否主导结果，本文还对 $M(0)$ 与 $\delta$ 进行补充 profile。结果显示，二者均存在较宽的可接受范围，其变化可被其他动力学参数补偿，当前数据不足以支持稳定的点估计。因此，固定 $M(0)=y(0)$ 与 $\delta=1$ 是避免引入弱可辨识参数、保持各序列比较口径一致的合理且实用设定。完整结果见附录 E。
...
\subsection{恐惧效应的机制路径依赖性}

在 fear-memory 繁殖抑制路径内，随着恐惧强度 $\phi$ 增大，通过长期收敛诊断的情景表现为猎物长期均值基本不变、捕食者长期均值下降，与正平衡关系的理论预测一致。$\phi\in[0,0.06]$ 的 31 个扫描情景在当前积分范围内均未触发灭绝判定，其中 20 个通过长期收敛诊断；其余 11 个情景不解释为已达到长期平衡。时间序列对照与 $\phi$ 扫描结果分别见附录图\,\ref{fig:main-ts} 和图\,\ref{fig:phi-scan}。

在已通过收敛诊断的 fear-memory 扫描范围内，增加 $\phi$ 未改变长期变化方向，但扩大了捕食者下降幅度。当相同的 20\% 标准化抑制通过不同机制进入方程时，种群变化方向也会发生改变。如图\,\ref{fig:mechanisms} 所示，三种繁殖抑制路径使捕食者长期均值下降约 16.7\%--18.0\%，猎物长期均值基本不变；觅食抑制与处理时间延长则同时提高猎物和捕食者长期均值。这表明恐惧效应的种群后果不仅取决于抑制强度，更取决于恐惧作用于何种生态过程。

\begin{figure}[H]
  \centering
  \imginclude[width=0.88\textwidth]{09_literature_mechanisms_bars.png}
  \caption{20\% 标准化抑制基准下，各恐惧机制相对无恐惧基线的长期均值变化。繁殖抑制路径降低捕食者长期均值，觅食抑制与处理时间延长则同时提高猎物和捕食者长期均值，表明种群响应具有明显的机制路径依赖性。}
  \label{fig:mechanisms}
\end{figure}

20\% 抑制仅用于建立共同的机制比较基准，不代表真实系统中的典型恐惧强度。不同路径匹配不同过程量，因此该比较用于判断变化方向与机制路径依赖性，不构成严格的跨机制效应量排序。
...
按相同口径扩展到全部六模型后，留后预测与 AICc 的最优模型均随序列变化，未出现一致胜出的恐惧通道。研究级聚合后的胜负仍然分散，说明研究内重复不改变这一主要结论。完整六模型胜负、零恐惧候选检查和研究级聚合结果见附录 D。
...
为量化胜负幅度，对每条序列计算最佳可比较恐惧模型相对 baseline 的留后 RMSE 改善率
\[
 I_s=100\frac{\mathrm{RMSE}_{\mathrm{baseline},s}
 -\min_{m\in\mathcal{F}}\mathrm{RMSE}_{m,s}}
 {\mathrm{RMSE}_{\mathrm{baseline},s}},
\]
其中 $\mathcal{F}$ 为五种 Holling II 恐惧模型，不包含 B-D；正值表示最佳恐惧模型优于 baseline。15 条序列中有 12 条数值改善，正改善序列的中位改善为 10.47\%；8/15 至少改善 1\%，7/15 至少改善 5\%，6/15 至少改善 10\%。另有 4 条正改善序列的幅度不足 1\%，表明仅统计胜负会夸大部分差异。改善率范围为 $-2.65\%$ 至 $72.23\%$；最大值来自包含未建模刺网移除干预的 Windermere North，因此不能解释为恐惧机制证据。

\begin{figure}[H]
  \centering
  \imginclude[width=0.92\textwidth]{seven_model_real_fits/report_protocol_six_model_holdout_improvement.png}
  \caption{最佳 Holling II 恐惧模型相对 baseline 的留后 RMSE 改善率。条形按改善率排序，颜色表示取得最低留后 RMSE 的恐惧通道；零线左侧表示所有可比较恐惧模型均劣于 baseline。}
  \label{fig:six-model-improvement}
\end{figure}

全部拟合均达到 success 或 usable\_limit 状态；其中超过半数为 usable\_limit，因此胜负统计仍包含优化不确定性。
...
\begin{table}[H]
 ...
 \begin{tabular}{p{0.16\textwidth}p{0.35\textwidth}p{0.35\textwidth}}
  ...
  \midrule
  结构区分与机制识别 & 数值实验支持不同机制结构可产生不同响应；真实数据能够比较候选结构的相对表现 & 真实系统中存在与某一特定恐惧通道对应的机制；数据偏好可以直接解释为机制证据\\
  \midrule
...
\begin{figure}[H]
  ...
  \caption{代表性拟合：Andrén 区 7 的 fear-memory 模型（baseline--fear-memory 两模型比较中 AICc 最优）。阴影分界后的观测用于连续多步留后验证。}
...
对 $\phi$、$M(0)$ 和 $\delta$ 的补充诊断表明，当前数据不足以稳定识别 fear-memory 的恐惧强度、初始记忆和记忆衰减尺度。较宽的可接受范围说明这些参数的变化可被其他动力学参数补偿，因此单个拟合值不应被解释为真实系统中的恐惧强度或记忆时间尺度。

这一结果也说明，固定 $M(0)=y(0)$ 与 $\delta=1$ 是合理且实用的统一处理：它避免在短序列中引入额外的弱可辨识参数，同时保持各序列之间的比较口径一致。本文关于机制差异的结论因此来自不同模型结构的相对表现，而非特定恐惧参数的点估计。扩边诊断、profile 图与完整数值结果见附录 E。

\subsection{讨论：证据能够支持什么}
\label{sec:discussion}

实验研究、概念框架与综合分析共同说明，恐惧效应在原理上可以改变种群动力学；本文的标准化实验进一步表明，这种改变取决于恐惧进入方程的具体路径。然而，真实数据比较回答的是候选模型能否描述和预测观测轨迹，而不是某种生态机制是否真实发生。本文中，含恐惧模型虽能改善部分序列的留后预测，但模型支持随序列和评价标准变化，恐惧相关参数也缺乏稳定识别。因此，预测能力与机制识别必须分开解释。
...
\textbf{机制层面：预测改善不能等同于恐惧机制的识别。}宏观捕食者--猎物时间序列可以检验某种动力学结构是否有助于描述和预测种群变化，却不能单独判定该结构所代表的生态过程是否真实发生。本文中，不同恐惧通道均能在部分序列上取得预测优势，但优势随研究系统而变化，现有数据也不足以稳定区分不同通道。因此，含恐惧模型应被理解为解释观测动力学的一组候选假设，而不是已经识别出的因果机制。要进一步确认恐惧通过繁殖、觅食或处理时间等具体路径作用于真实种群，还需要行为观测、风险操纵实验或相应过程量的独立测量。

综合来看，本文的贡献在于：(1) 展示共同抑制基准下种群响应的机制路径依赖性；(2) 通过公开时间序列比较表明，没有一个恐惧通道能够稳定优于无恐惧基线；(3) 明确宏观种群时间序列的证据边界，并指出识别具体恐惧路径仍需要行为观测、风险操纵实验或过程量的独立测量。
...
表\,\ref{tab:six-model-improvement} 给出与图\,\ref{fig:six-model-improvement} 对应的逐序列数值。绝对改善定义为 baseline 留后 RMSE 减去最佳恐惧模型留后 RMSE。

\begin{landscape}
\begin{longtable}{p{0.20\linewidth}p{0.20\linewidth}rrrr}
 \caption{最佳恐惧模型相对 baseline 的逐序列留后 RMSE 改善}\label{tab:six-model-improvement}\\
 \toprule
 序列 & 最佳恐惧模型 & baseline RMSE & 最佳恐惧 RMSE & 绝对改善 & 改善率（\%）\\
 \midrule
 \endfirsthead
 \toprule
 序列 & 最佳恐惧模型 & baseline RMSE & 最佳恐惧 RMSE & 绝对改善 & 改善率（\%）\\
 \midrule
 \endhead
 GLERL & fear-handling & 0.2814 & 0.2793 & 0.0021 & 0.75\\
 Andrén-1 & fear-memory & 0.0702 & 0.0652 & 0.0049 & 7.03\\
 Andrén-2 & fear-memory & 0.0968 & 0.0833 & 0.0135 & 13.91\\
Andrén-3 & fear-foraging & 0.0755 & 0.0755 & $<0.0001$ & $<0.01$\\
 Andrén-4 & fear-foraging & 0.0652 & 0.0417 & 0.0235 & 36.00\\
 Andrén-5 & fear-instant & 0.0602 & 0.0602 & 0.0000 & 0.00\\
 Andrén-6 & fear-handling & 0.1810 & 0.1789 & 0.0021 & 1.17\\
 Andrén-7 & fear-handling & 0.2429 & 0.1298 & 0.1131 & 46.57\\
 WRHW & fear-instant & 0.1534 & 0.1266 & 0.0268 & 17.45\\
 TP & fear-handling & 0.1030 & 0.0801 & 0.0229 & 22.21\\
WRGP & fear-memory & 0.1862 & 0.1862 & $<0.0001$ & $<0.01$\\
Isle Royale & fear-memory & 0.2283 & 0.2283 & $<0.0001$ & $<0.01$\\
 Windermere North & fear-instant & 1.5234 & 0.4231 & 1.1004 & 72.23\\
 Windermere South & fear-instant & 1.1560 & 1.1700 & -0.0140 & -1.21\\
 Komi lynx--hare & fear-foraging & 0.5811 & 0.5964 & -0.0154 & -2.65\\
 \bottomrule
\end{longtable}
\end{landscape}
...
\begin{figure}[H]
 ...
 \begin{subfigure}{0.48\textwidth}
  ...
  \caption{Andrén-7：memory 在两模型比较中的 AICc 最优}
 \end{subfigure}\hfill
 \begin{subfigure}{0.48\textwidth}
...
$M(0)$ 与 $\delta$ 的代表 profile 如下。表中固定 $\delta=1$ 的数值来自 profile 阶段的局部重新优化，不是正式多起点拟合结果的直接复现，因此个别序列与附录 D 的正式 fear-memory 指标略有差异。
...
\begin{figure}[H]
 ...
 \begin{subfigure}{0.32\textwidth}
  ...
  \caption{TP}
 \end{subfigure}
 \caption{$M(0)/y(0)$ profile 的三条代表序列。宽而平坦的训练 profile 表明初始记忆弱可辨识。}
...
\begin{figure}[H]
 ...
 \begin{subfigure}{0.32\textwidth}
  ...
  \caption{TP}
 \end{subfigure}
 \caption{$\delta$ profile 的三条代表序列。宽可接受范围表明记忆衰减尺度缺乏稳定识别。}
...
\begin{figure}[H]
 \centering
  \imginclude[width=0.78\textwidth]{08_literature_mechanisms_timeseries.png}
