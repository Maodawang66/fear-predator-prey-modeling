...
表\,\ref{tab:wins} 汇总核心三模型（baseline、fear-memory、fear-instant）的序列级预测结果。fear-memory 在 6 条序列的留后验证上单独最优，fear-instant 在 5 条上最优，baseline 在 3 条上最优；但 baseline 在 10 条序列上取得最低 AICc。GLERL 的三模型训练和验证 RMSE 相同，不能区分三种结构。该结果说明，短期留后预测改善与复杂度校正后的模型支持并不等价。按研究聚合后（Andrén 七区取中位数、Killifish 三站取中位数、Windermere 两湖盆取中位数，其余独立研究各算 1 单位，共约 6 个研究级单位），模型胜负分布依然不支持任何通道的普遍优越性（完整研究级胜负见附录 D）。
...
按相同训练段归一化、20\% 连续多步留后和固定种子多起点优化口径扩展到全部六模型（新增 fear-saturating、fear-foraging、fear-handling）后，90 次拟合均可比较。序列级留后验证单独最优的分布为：fear-memory 5 条、baseline 3 条、fear-instant 3 条、fear-handling 2 条、fear-foraging 1 条，GLERL 六模型并列。序列级 AICc 最优分布为：baseline 8 条、fear-foraging 3 条、fear-instant 2 条、fear-memory 1 条、fear-saturating 1 条。该扩展仍不支持”某一种恐惧通道普遍正确”：不同序列由不同通道获胜，且预测胜出不能替代参数可辨识性证据。
